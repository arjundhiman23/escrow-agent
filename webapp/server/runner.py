...
from webapp.server.storage import read_meta, write_meta  # noqa
...
def _log(st, run_id, meta, line):
    meta["log"].append(f"[{_now()}] {line}")
    write_meta(st, run_id, meta)


def _stage(st, run_id, meta, stage):
    meta["stage"] = stage
    meta["stages_done"] = STAGES[:STAGES.index(stage)]
    _log(st, run_id, meta, f"stage -> {stage}")


def execute_run(st, run_id):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for f in meta["inputs"]["statements"] + [meta["inputs"]["catra_template"], meta["inputs"]["tra_template"]]:
                with open(os.path.join(ind, f), "wb") as fh:
                    fh.write(st.get_bytes(f"runs/{run_id}/inputs/{f}"))

            _stage(st, run_id, meta, "INGEST")
            txns = []
            for f in sorted(meta["inputs"]["statements"]):
                ...
                txns += rows
                _log(st, run_id, meta, f"{f} -> {q}: {len(rows)} main-account transactions")

            _stage(st, run_id, meta, "CLASSIFY")
            ...
            _log(st, run_id, meta, f"classified {len(txns)} transactions; {n_review} flagged for review")
            if ai_assist:
                u = tracker.summary()
                _log(st, run_id, meta,
            ...
            _stage(st, run_id, meta, "VALIDATE")
            ...
            _log(st, run_id, meta, f"run completed — verdict {verdict}")
